...
from mody.Config import Config
from mody.core.decorators import check_owner
from mody.helpers import humanbytes, progress
from mody.helpers.utils import _zeutils
...
# BACK
@zeub.tgbot.on(CallbackQuery(pattern="fmback"))
@check_owner
...
# UP
@zeub.tgbot.on(CallbackQuery(pattern="fmup_(.*)"))
@check_owner
async def up(event):
...
# DOWN
@zeub.tgbot.on(CallbackQuery(pattern="fmdown_(.*)"))
@check_owner
...
# FORTH
@zeub.tgbot.on(CallbackQuery(pattern="fmforth_(.*)"))
@check_owner
...
# REMOVE
@zeub.tgbot.on(CallbackQuery(pattern="fmrem_(.*)"))
@check_owner
async def remove(event):
    ...
    await asyncio.sleep(1)
    await event.edit(msg, buttons=buttons)
    await _zeutils.runcmd(f"rm -rf '{rpath}'")
    await event.answer(f"- المسـار {rpath} تم حذفـه .. بنجـاح ✓")


# SEND
@zeub.tgbot.on(CallbackQuery(pattern="fmsend"))
@check_owner
...
# CUT
@zeub.tgbot.on(CallbackQuery(pattern="fmcut_(.*)"))
@check_owner
async def cut(event):
...
# COPY
@zeub.tgbot.on(CallbackQuery(pattern="fmcopy_(.*)"))
@check_owner
...
# PASTE
@zeub.tgbot.on(CallbackQuery(pattern="fmpaste_(.*)"))
@check_owner
