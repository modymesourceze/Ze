...
# =========================================================== #
#                                      مـــودي الهيبـــه - T.me/D_S_I                                  #
# =========================================================== #


#Write Code By T.me/D_S_I
@zeub.ze_cmd(pattern="repo(?:\s|$)([\s\S]*)")
async def mody2(event):
    mody = event.pattern_match.group(1)
    chat = "@GitHub_Download_robot"
    reply_id_ = await reply_id(event)
    sourceze = await edit_or_reply(event, REPO_SEARCH_STRING, parse_mode="html")
    async with event.client.conversation(chat) as conv:
        try:
            await conv.send_message("/start")
            await conv.get_response()
            purgeflag = await conv.send_message(mody)
        ...
        if not repo.document:
            return await edit_delete(sourceze, REPO_NOT_FOUND, parse_mode="html")
        await event.client.send_read_acknowledge(conv.chat_id)
        await event.client.send_file(
            event.chat_id,
            repo,
            caption=f"<b>⎉╎الريبـو :- <code>{mody}</code></b>\n<b>⎉╎تم التحميـل .. بنجـاح ✅</b>",
            parse_mode="html",
            reply_to=reply_id_,
        )
