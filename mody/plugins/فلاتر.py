...
from mody import Convert, zeub

from ..core.managers import edit_delete, edit_or_reply
from ..helpers import asciiart, ze_meeme, ze_meme, media_type, meme_type
...
async def memes(event):
    ...
    zeinput = event.pattern_match.group(2)
    reply = await event.get_reply_message()
    ...
    if gvarstatus("ZE_FONTS") is None:
        ZE_FONTS = "mody/helpers/styles/Terserah.ttf"
    else:
        ZE_FONTS = gvarstatus("ZE_FONTS")
    if max(len(top), len(bottom)) < 21:
        await ZE_meme(ZE_FONTS, top, bottom, meme_file, meme)
    else:
        await ze_meeme(top, bottom, ZE_FONTS, meme_file, meme)
...
@zeub.ze_cmd(
    ...
    info={
        "header": "لعـرض قائمـة خطـوط زد إي",
        "الاستخـدام": "{tr}.الخطوط",
    },
)
async def lang(event):
    "لعـرض قائمـة خطـوط زد إي"
    input_str = event.pattern_match.group(1)
    if not input_str:
        await event.edit(f"**☣╎قائمـة خطـوط زد إي هـي :-**\n**قم بنسخ اسم الخط ثم ارسل (.خط + اسم الخط)**\n\n{FONTS}")
        return
    else:
        return
...
async def lang(event):
    ...
    if not input_str:
        ...
        return
    if input_str == "عربي":
        arr = f"mody/helpers/styles/zarz.ttf"
        addgvar("ZE_FONTS", arr)
    ...
    if input_str not in font_list:
        ...
        await catevent.edit(f"**☣╎قائمـة خطـوط زد إي هـي :-**\n\n{FONTS}")
    else:
        arg = f"mody/helpers/styles/{input_str}"
        addgvar("ZE_FONTS", arg)
...
async def memes(event):
    ...
    zeinput = event.pattern_match.group(1)
    reply = await event.get_reply_message()
    if not reply:
        return await edit_delete(event, "**☣╎قم بالـرد ع وسـائط مدعومـه ...**")
    san = base64.b64decode("QUFBQUFGRV9vWjVYVE5fUnVaaEtOdw==")
...
async def memes(event):
    ...
    zeinput = event.pattern_match.group(1)
    zeinput = int(zeinput) if zeinput else 50
    reply = await event.get_reply_message()
    if not reply:
        return await edit_delete(event, "**☣╎قم بالـرد ع وسـائط مدعومـه ...**")
    san = base64.b64decode("QUFBQUFGRV9vWjVYVE5fUnVaaEtOdw==")
    ...
    try:
        await crop(meme_file, outputfile, zeinput)
    except Exception as e:
...
async def memes(event):
    ...
    try:
        await add_frame(meme_file, outputfile, zeinput, colr)
    except Exception as e:
        return await output[0].edit(f"`{e}`")
    try:
        await event.client.send_file(
