...
from mody.utils import admin_cmd
...
async def lol(event):
    ...
    try:
        await zeub(functions.channels.JoinChannelRequest(bol))
        await zzz.edit("**- تم الانضمـام .. بنجـاح  ✓**")
    except Exception as e:
        await zzz.edit(str(e))


@zeub.ze_cmd(pattern="غادر ([\s\S]*)")
async def lol(event):
    a = event.text
    bol = a[6:]
    ...
    zzz = await event.reply(sweetie, parse_mode=None, link_preview=None)
    try:
        await zeub(functions.channels.LeaveChannelRequest(bol))
        await zzz.edit("**- تم المغـادرة .. بنجـاح  ✓**")
    except Exception as e:
        await zzz.edit(str(e))


@zeub.ze_cmd(pattern="اضافه ([\s\S]*)")
async def _(event):
