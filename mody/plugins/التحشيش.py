...
async def ge(user, event):
    ...
    try:
        ...
        return None
    return user_obj
########################  Ze Userbot ~ By: Mody (@D_S_I)  ########################
@zeub.ze_cmd(pattern="رابط الحذف")
async def _(ze):
    await edit_or_reply (ze, "𓆰 [🔱 𝐒𝐎𝐔𝐑𝐂𝐄 𝐙𝐄 🔱 - 𝘿𝙀𝙇𝙀𝙏𝙀](t.me/source_ze) 🗑♻️𓆪\n**𓍹━─━─━─━─🔱 𝐙𝐄 🔱─━─━─━─━𓍻**\n\n **✵│رابـط الحـذف ↬** https://telegram.org/deactivate \n\n\n **✵│بـوت الحـذف¹  ↬** @LC6BOT\n**✵│بـوت الحـذف²  ↬** @DTeLebot")
########################  Ze Userbot ~ By: Mody (@D_S_I)  ########################
@zeub.ze_cmd(pattern="بوت الحذف")
async def _(ze):
    await edit_or_reply (ze, "𓆰 [🔱 𝐒𝐎𝐔𝐑𝐂𝐄 𝐙𝐄 🔱 - 𝘿𝙀𝙇𝙀𝙏𝙀](t.me/source_ze) 🗑♻️𓆪\n**𓍹━─━─━─━─🔱 𝐙𝐄 🔱─━─━─━─━𓍻**\n\n **✵│رابـط الحـذف ↬** https://telegram.org/deactivate \n\n\n **✵│بـوت الحـذف¹  ↬** @LC6BOT\n**✵│بـوت الحـذف²  ↬** @DTeLebot")
########################  Ze Userbot ~ By: Mody (@D_S_I)  ########################
@zeub.ze_cmd(pattern="رفع جلب(?: |$)(.*)")
async def permalink(malath):
    user, custom = await get_user_from_event(malath)
    if not user:
        return
    if user.id in ze_dev:
        return await edit_or_reply(malath, f"**╮ ↻ عـذراً .. لا يمكننـي اهانـة المطـورين  ♪╰**")
    if user.id in mod_dev:
...
########################  Ze Userbot ~ By: Mody (@D_S_I)  ########################
ModyTHS_cmd = (
    "𓆩 [🔱 𝗦𝗼𝘂𝗿𝗰𝗲 𝐙𝐄 𝗖𝗼𝗻𝗳𝗶𝗴 𝗩𝗮𝗿𝘀 🔱 - اوامـر التحشيش](t.me/source_ze) 𓆪\n\n"
...
@zeub.ze_cmd(pattern="كت(?: |$)(.*)")
async def ahmed(ahmed): # Code Update by @D_S_I
    mody = ahmed.pattern_match.group(1)
    zilzal = await ahmed.get_reply_message()
    mo_dy = random.choice(kettuet)
    if not zilzal and not mody: # Code Update by @D_S_I
        return await edit_or_reply(ahmed, f"**⌔╎{mo_dy}**")
    user, custom = await get_user_from_event(ahmed)
    if not user: # Code Update by @D_S_I
        return
    sourceze2 = user.first_name.replace("\u2060", "") if user.first_name else user.username
    if custom: # Code Update by @D_S_I
        sourceze2 = custom
    me = await ahmed.client.get_me()
    my_first = me.first_name
    my_ahmed = f"[{me.first_name}](tg://user?id={me.id})"
    await edit_or_reply(ahmed, f"**⌔╎لـ  ** [{sourceze2}](tg://user?id={user.id}) \n**⌔╎{mo_dy}**")

@zeub.ze_cmd(pattern="(نسبه الحب|نسبة الحب)(?: |$)(.*)")
async def ze(malath):
    user, custom = await get_user_from_event(malath)
    if not user:
        return
    sourceze = user.first_name.replace("\u2060", "") if user.first_name else user.username
    zet = random.choice(mth)
    await edit_or_reply(malath, f"**☣╎نـسبـة حبكـم انـت و**  [{sourceze}](tg://user?id={user.id}) **هـي {zet} 😻♥️**")

@zeub.ze_cmd(pattern="(نسبه الانوثة|نسبة الانوثه|نسبه الانوثه|نسبة الانوثة)(?: |$)(.*)")
async def ze(malath):
    user, custom = await get_user_from_event(malath)
    if not user:
        return
    ...
    if user.id in mod_dev:
        return await edit_or_reply(malath, f"**╮ ↻ عـذراً .. لا يمكننـي اهانـة مطـور السـورس  ♪╰**")
    sourceze = user.first_name.replace("\u2060", "") if user.first_name else user.username
    zet = random.choice(zid)
    await edit_or_reply(malath, f"**☣╎نسبـة الانوثه لـ**  [{sourceze}](tg://user?id={user.id}) **هـي {zet} 🤰**")

@zeub.ze_cmd(pattern="(نسبه الغباء|نسبة الغباء)(?: |$)(.*)")
async def permalink(malath):
    user, custom = await get_user_from_event(malath)
    if not user:
        return
    ...
    sourceze = user.first_name.replace("\u2060", "") if user.first_name else user.username
    zet = random.choice(zid)
    await edit_or_reply(malath, f"**☣╎نسبـة الغبـاء لـ**  [{sourceze}](tg://user?id={user.id}) **هـي {zet} 😂💔**")

@zeub.ze_cmd(pattern="(نسبه الانحراف|نسبة الانحراف)(?: |$)(.*)")
async def permalink(malath):
    user, custom = await get_user_from_event(malath)
    if not user:
        return
    ...
    sourceze = user.first_name.replace("\u2060", "") if user.first_name else user.username
    zet = random.choice(zid)
    await edit_or_reply(malath, f"**☣╎نسبـة الانحـراف لـ**  [{sourceze}](tg://user?id={user.id}) **هـي {zet} 🥵🖤**")

@zeub.ze_cmd(pattern="(نسبه المثليه|نسبة المثليه)(?: |$)(.*)")
async def permalink(malath):
    user, custom = await get_user_from_event(malath)
    if not user:
        return
    ...
    sourceze = user.first_name.replace("\u2060", "") if user.first_name else user.username
    zet = random.choice(zid)
    await edit_or_reply(malath, f"**☣╎نسبـة المثليـه لـ**  [{sourceze}](tg://user?id={user.id}) **هـي {zet} 🤡 🏳️‍🌈.**")

@zeub.ze_cmd(pattern="(نسبه النجاح|نسبة النجاح)(?: |$)(.*)")
async def permalink(malath):
    user, custom = await get_user_from_event(malath)
    if not user:
        return
    sourceze = user.first_name.replace("\u2060", "") if user.first_name else user.username
    zet = random.choice(zid)
    await edit_or_reply(malath, f"**☣╎نسبـة النجـاح لـ** [{sourceze}](tg://user?id={user.id}) **هـي {zet} 🤓.**") 

@zeub.ze_cmd(pattern="(نسبه الكراهية|نسبة الكراهيه|نسبه الكراهيه|نسبة الكراهية)(?: |$)(.*)")
async def permalink(malath):
    user, custom = await get_user_from_event(malath)
    if not user:
        return
    sourceze = user.first_name.replace("\u2060", "") if user.first_name else user.username
    zet = random.choice(zid)
    await edit_or_reply(malath, f"**☣╎نسبـة الكراهيـة لـ** [{sourceze}](tg://user?id={user.id}) **هـي {zet} 🤮.**")

@zeub.ze_cmd(pattern="رفع ورع(?: |$)(.*)")
async def ze(malath):
    user, custom = await get_user_from_event(malath)
    if not user:
        return
    ...
    if user.id in mod_dev:
        return await edit_or_reply(malath, f"**╮ ↻ عـذراً .. لا يمكننـي اهانـة مطـور السـورس  ♪╰**")
    sourceze2 = user.first_name.replace("\u2060", "") if user.first_name else user.username
    await edit_or_reply(malath, f"**🚹 ╎ المستخـدم  ➥ • ** [{sourceze2}](tg://user?id={user.id}) \n☑️ **╎تم رفعـه ورع القـروب 😹🙇🏻.**")

@zeub.ze_cmd(pattern="رفع مزه(?: |$)(.*)")
async def ze(malath):
    user, custom = await get_user_from_event(malath)
    if not user:
        return
    ...
    if user.id in mod_dev:
        return await edit_or_reply(malath, f"**╮ ↻ عـذراً .. لا يمكننـي اهانـة مطـور السـورس  ♪╰**")
    sourceze2 = user.first_name.replace("\u2060", "") if user.first_name else user.username
    await edit_or_reply(malath, f"**🚺 ╎ الحلـوه ➥ • ** [{sourceze2}](tg://user?id={user.id}) \n☑️ **╎ تم رفعـها مـزة الكروب 🥳💃.**")

@zeub.ze_cmd(pattern="رفع مطي(?: |$)(.*)")
async def ze(malath):
    user, custom = await get_user_from_event(malath)
    if not user:
        return
...
async def permalink(malath):
    ...
    sourceze2 = user.first_name.replace("\u2060", "") if user.first_name else user.username
    await edit_or_reply(
        malath, f"**🚹 ╎ المستخـدم  ➥ • ** [{sourceze2}](tg://user?id={user.id}) \n☑️ **🐈╎ تم رفعـه بـزون .** \n"
    )

@zeub.ze_cmd(pattern="رفع زباله(?: |$)(.*)")
async def ze(malath):
    user, custom = await get_user_from_event(malath)
    if not user:
        return
...
@zeub.ze_cmd(pattern="رفع مرتبطه(?: |$)(.*)")
async def ze(malath):
    user, custom = await get_user_from_event(malath)
    if not user:
        return
...
@zeub.ze_cmd(pattern="رفع حبيبي(?: |$)(.*)")
async def ze(malath):
    user, custom = await get_user_from_event(malath)
    if not user:
        return
    sourceze2 = user.first_name.replace("\u2060", "") if user.first_name else user.username
    await edit_or_reply(malath, f"**🚹 ╎ الحلـو  ➥ • ** [{sourceze2}](tg://user?id={user.id}) \n☑️ **╎ تم رفعـه حبيبـج .. بنجـاح 💍🤵‍♂👰🏻‍♀.**")

@zeub.ze_cmd(pattern="رفع خطيبتي(?: |$)(.*)")
async def ze(malath):
    user, custom = await get_user_from_event(malath)
    if not user:
        return
...
async def permalink(malath):
    ...
    if user.id in mod_dev:
        return await edit_or_reply(malath, f"**╮ ↻ لك هـذا مـــودي الهيبـه . . شما حجيت ماكـدر اوصفـه 🙊💘 ♪╰**")
    tag = user.first_name.replace("\u2060", "") if user.first_name else user.username
    owsf = random.choice(wasf)
    await edit_or_reply(malath, f"**- {owsf}**")

@zeub.ze_cmd(pattern="هينه(?: |$)(.*)")
async def permalink(malath):
    user, custom = await get_user_from_event(malath)
    if not user:
        return
...
# Copyright (C) 2022 ze-Thon . All Rights Reserved
@zeub.ze_cmd(pattern="التحشيش")
async def cmd(modyyy):
    await edit_or_reply(modyyy, ModyTHS_cmd)
