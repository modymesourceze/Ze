...
async def ban_user(chat_id, i, rights):
    try:
        await modub(functions.channels.EditBannedRequest(chat_id, i, rights))
        return True, None
    except Exception as exc:
        return False, str(exc)

@modub.mod_cmd(pattern=r"اطردني(.*)")
async def kickme(leave):
    await leave.edit("**☣╎جـاري مـغادرة المجـموعة مـع السـلامة  🚶‍♂️  ..**")
    await leave.client.kick_participant(leave.chat_id, "me")

@modub.mod_cmd(pattern=r"مغادره(.*)")
async def banme(leave):
    await leave.edit("**☣╎جـاري مـغادرة المجـموعة مـع السـلامة  🚶‍♂️  ..**")
    await leave.client.kick_participant(leave.chat_id, "me")

@modub.mod_cmd(pattern="بوتي$")
async def _(event):
    TG_BOT_USERNAME = Config.TG_BOT_USERNAME
    await event.reply(f"**☣╎البـوت المسـاعد الخـاص بك هـو** \n {TG_BOT_USERNAME}")

@modub.mod_cmd(pattern="حالتي ?(.*)")
async def zee(event):
...
@modub.on(events.NewMessage(pattern="/ze"))
async def _(event):
...
async def _(event):
    ...
    modevent = await edit_or_reply(event, "**. . .**")
...
@modub.mod_cmd(pattern="(مغادرة الكروبات|مغادرة المجموعات)")
async def leave_groups(event):
    ...
    try:
        ...
        for group in gg:
            if group not in ss:
                await modub.delete_dialog(group)
                num += 1
                await sleep(1)
        if num >=1:
...
@modub.mod_cmd(pattern="مغادرة القنوات")
...
@modub.mod_cmd(pattern="حذف الخاص")
async def _(event):
...
@modub.mod_cmd(pattern="حذف البوتات")
async def _(event):
...
@modub.mod_cmd(pattern="تفليش بالبوت$", groups_only=True)
async def banavot(event):
    chat_id = event.chat_id
    is_admin = False
    try:
        await modub(GetParticipantRequest(event.chat_id, event.sender_id))
    except UserNotParticipantError:
        pass
    spam_chats.append(chat_id)
    async for usr in modub.iter_participants(chat_id):
        ...
        await modub.send_message(chat_id, usrtxt)
...
@modub.mod_cmd(pattern=f"{HDRALL}$", groups_only=True)
async def banavot(event):
    chat_id = event.chat_id
    is_admin = False
    try:
...
@modub.mod_cmd(pattern=f"{KTMALL}$", groups_only=True)
async def banavot(event):
    chat_id = event.chat_id
    is_admin = False
    try:
...
@modub.mod_cmd(pattern="الغاء التفليش", groups_only=True)
...
@modub.mod_cmd(pattern="ايقاف التفليش", groups_only=True)
async def unbanbot(event):
    if not event.chat_id in spam_chats:
        return await edit_or_reply(event, "**- لاتوجـد عمليـة تفليـش هنـا لـ إيقافـها ؟!**")
    else:
