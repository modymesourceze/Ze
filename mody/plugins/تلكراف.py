...
from . import BOTLOG, BOTLOG_CHATID, zeub, reply_id
...
async def _(event):
    ...
    zeevent = await edit_or_reply(event, "** ➥ جاري المعالجه ༗...**")
    if BOTLOG:
    ...
    if input_str in ["ميديا", "m"]:
        ...
        await zeevent.edit(f"** ➥ تم تحميل** {downloaded_file_name} **.. بنجـاح**")
        if downloaded_file_name.endswith((".webp")):
            resize_image(downloaded_file_name)
        try:
            media_urls = upload_file(downloaded_file_name)
        except exceptions.TelegraphException as exc:
            await zeevent.edit(f"**- خطـأ : **\n`{exc}`")
...
async def ctg(event):
    ...
    zeevent = await edit_or_reply(event, "```Processing...```")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await zeub(unblock("chotamreaderbot"))
            msg_flag = await conv.send_message(urls[0])
        response = await conv.get_response()
        await event.client.send_read_acknowledge(conv.chat_id)
        if response.text.startswith(""):
            await edit_or_reply(zeevent, "Am I Dumb Or Am I Dumb?")
        else:
