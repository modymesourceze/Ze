...
from . import CMD_HELP, CMD_LIST, SUDO_LIST, zeub, edit_delete, edit_or_reply, reply_id
...
def plug_checker(plugin):
    plug_path = f"./mody/plugins/{plugin}.py"
    return plug_path
...
@zeub.ze_cmd(
    ...
    info={
        ...
        "الوصـف": "بالـرد ع اي ملف (يدعم سورس زد إي) لـ تنصيبه في بوتك.",
        "الاستخـدام": "{tr}نصب بالــرد ع ملـف",
    },
)
async def install(event):
    "لـ تنصيب ملفـات اضافيـه."
    mody = event.sender_id
    ze_dev = (6581896306)
    if mody not in ze_dev:
