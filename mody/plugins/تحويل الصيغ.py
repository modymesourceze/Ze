...
from ..helpers.utils import _zetools, _zeutils, _format
from . import zeub, make_gif, progress
...
async def pic_gifcmd(event):  # sourcery no-metrics
    ...
    output = await _zetools.media_to_pic(event, reply)
    if output[1] is None:
...
@zeub.ze_cmd(pattern="ملصق متحرك(?: |$)(.*)")
...
@zeub.ze_cmd(pattern="ممتحركة(?: |$)((-)?(r|l|u|d|s|i)?)$")
async def pic_gifcmd(event):
    ...
    imag = await _zetools.media_to_pic(event, reply)
    if imag[1] is None:
        return await edit_delete(
