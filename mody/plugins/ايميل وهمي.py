...
# code by t.me/D_S_I
@zeub.ze_cmd(pattern="بريد$")
async def zelzal_gpt(event):
    chat = "@TeMail_Robot" # code by t.me/D_S_I
    ze = await edit_or_reply(event, "**☣╎جـار إنشـاء ايميـل وهمـي 📧...**")
    async with borg.conversation(chat) as conv: # code by t.me/D_S_I
        try:
            ...
            malath = ze.text
            if "📧 Your temporary email" in ze.text:
            ...
            await zeub(unblock("TeMail_Robot"))
            await conv.send_message("/start")
            await conv.get_response()
            await conv.send_message("📧 Generate Email")
            await asyncio.sleep(5)
...
# code by t.me/D_S_I
@zeub.ze_cmd(pattern="الوارد$")
async def zelzal_gpt(event):
    chat = "@TeMail_Robot" # code by t.me/D_S_I
    ze = await edit_or_reply(event, "**☣╎جـار جلب رسائـل البريـد 📬...**")
    async with borg.conversation(chat) as conv: # code by t.me/D_S_I
        try:
            await conv.send_message("/start")
            await conv.get_response()
