...
async def variable(var):  # sourcery no-metrics
    ...
    if exe == "get":
        ze = await edit_or_reply(var, "**⌔∮ جاري الحصول على المعلومات. **")
        await asyncio.sleep(1.0)
        try:
            variable = var.pattern_match.group(2).split()[0]
            if variable in heroku_var:
                return await ze.edit(
                    "𓆩 🔱 𝐒𝐎𝐔𝐑𝐂𝐄 𝐙𝐄 🔱 - 𝑮𝑶𝑵𝑭𝑰𝑮 𝑽𝑨𝑹𝑺 𓆪\n**𓍹ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ𓍻**" f"\n\n**⌔∮الفــار** `{variable} = {heroku_var[variable]}` .\n"
                )
            await ze.edit(
        ...
        ze = await edit_or_reply(var, "**⌔∮ جاري اعداد المعلومات**")
        if not variable:
            return await ze.edit("`.set var <ConfigVars-name> <value>`")
        ...
        if variable in heroku_var:
            await ze.edit("**⌔∮ تم تغيـر** `{}` **:**\n **- المتغير :** `{}` \n**- يتم الان اعـادة تشغيـل بـوت زد إي يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(variable, value))
        else:
            await ze.edit("**⌔∮ تم اضافه** `{}` **:** \n**- المضاف اليه :** `{}` \n**يتم الان اعـادة تشغيـل بـوت زد إي يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(variable, value))
        heroku_var[variable] = value
    elif exe == "del":
        ze = await edit_or_reply(var, "⌔∮ الحصول على معلومات لحذف المتغير. ")
        try:
            variable = var.pattern_match.group(2).split()[0]
        except IndexError:
            return await ze.edit("⌔∮ يرجى تحديد `Configvars` تريد حذفها. ")
        await asyncio.sleep(1.5)
        if variable not in heroku_var:
            return await ze.edit(f"⌔∮ `{variable}`**  غير موجود**")

        await ze.edit(f"**⌔∮** `{variable}`  **تم حذفه بنجاح. \n**يتم الان اعـادة تشغيـل بـوت زد إي يستغـرق الامر 2-1 دقيقـه ▬▭ ...**")
