...
from . import ALIVE_NAME, BOTLOG, BOTLOG_CHATID, zeub, edit_delete, get_user_from_event
...
@zeub.ze_cmd(pattern="(تقليد|ازعاج)$")
async def echo(event):
    if event.reply_to_msg_id is None:
        return await edit_or_reply(event, "**- بالـرد على الشخص الذي تريد ازعاجـه**")
    zeubevent = await edit_or_reply(event, "**- يتم تفعيل هذا الامر انتظر قليلا ..**")
    user, rank = await get_user_from_event(event, zeubevent, nogroup=True)
    if not user:
        return
    ...
    try:
        ...
        await edit_delete(zeubevent, f"**☣╎خطـأ :**\n`{str(e)}`")
    else:
        await edit_or_reply(
            zeubevent,
...
@zeub.ze_cmd(pattern="(الغاء تقليد|ايقاف التقليد|ايقاف تقليد|الغاء الازعاج|ايقاف الازعاج|ايقاف ازعاج|الغاء ازعاج)$")
async def echo(event):
    if event.reply_to_msg_id is None:
...
@zeub.ze_cmd(pattern="حذف المقلدهم( للكل)?")
async def echo(event):
...
@zeub.ze_cmd(pattern="المقلدهم( للكل)?$")
async def echo(event):
    input_str = event.pattern_match.group(1)
    private_chats = ""
...
@zeub.ze_cmd(incoming=True, edited=False)
