...
async def zegban(event):  # sourcery no-metrics
    "To ban user in every group where you are admin."
    zee = await edit_or_reply(event, "**╮ ❐... جـاࢪِ حـظـࢪ الشخـص عـام**")
    start = datetime.now()
    user, reason = await get_user_from_event(event, zee)
    if not user:
        return
    if user.id == zeub.uid:
        return await edit_delete(zee, "**☣╎عـذراً ..لا استطيـع حظـࢪ نفسـي **")
    if user.id in ze_dev:
        return await edit_delete(zee, "**☣╎عـذراً ..لا استطيـع حظـࢪ احـد المطـورين عـام **")
    if user.id == 6581896306:
        return await edit_delete(zee, "**☣╎عـذراً ..لا استطيـع حظـࢪ مطـور السـورس عـام **")
    ...
    if gban_sql.is_gbanned(user.id):
        ...
        gban_sql.zegban(user.id, reason)
    san = await admin_groups(event.client)
    count = 0
    sandy = len(san)
    if sandy == 0:
        return await edit_delete(zee, "**☣╎عــذراً .. يجـب ان تكــون مشـرفـاً فـي مجموعـة واحـده ع الأقــل **")
    await zee.edit(
    ...
    zetaken = (end - start).seconds
    if reason:
        await zee.edit(
            f"**☣╎المستخـدم :** [{user.first_name}](tg://user?id={user.id})\n\n**☣╎تم حـظـࢪه عـام مـن {count} كــࢪوب خـلال {zetaken} ثـانيـه**\n**☣╎السـبب :** {reason}"
        )
    else:
        await zee.edit(
            f"**╮ ❐... الشخـص :** [{user.first_name}](tg://user?id={user.id})\n\n**╮ ❐... تـم حـظـࢪه عـام مـن {count} كــࢪوب خـلال {zetaken} ثـانيـه**"
        )
    if BOTLOG and count != 0:
        ...
        if reason:
            await event.client.send_message(
                ...
                \n**- الــوقت المسـتغــࢪق :** {zetaken} **ثــانيـه**",
            )
        else:
            await event.client.send_message(
                BOTLOG_CHATID,
...
async def zegban(event):
    "To unban the person from every group where you are admin."
    zee = await edit_or_reply(event, "**╮ ❐  جـاري الغــاء الحظـر العــام ❏╰**")
    start = datetime.now()
    user, reason = await get_user_from_event(event, zee)
    if not user:
        return
    ...
    if reason:
        await zee.edit(
            f"**☣╎المستخـدم :** [{user.first_name}](tg://user?id={user.id})\n\n**☣╎تم الغــاء حـظـࢪه عـام مـن {count} كــࢪوب خـلال {zetaken} ثـانيـه**\n**☣╎السـبب :** {reason}"
        )
    else:
        await zee.edit(
            f"**☣╎المستخـدم :** [{user.first_name}](tg://user?id={user.id})\n\n**☣╎تم الغــاء حـظـࢪه عـام مـن {count} كــࢪوب خـلال {zetaken} ثـانيـه**"
...
@zeub.ze_cmd(pattern=f"{KTMZ}(?:\s|$)([\s\S]*)")
...
@zeub.ze_cmd(pattern="الغاء كتم(?:\s|$)([\s\S]*)")
...
@zeub.ze_cmd(incoming=True, forword=True)
async def watcher(event):
    if is_muted(event.sender_id, "gmute"):
        await event.delete()


@zeub.ze_cmd(incoming=True)
async def watcher(event):
    if is_muted(event.sender_id, "gmute"):
        await event.delete()
...
async def catgkick(event):  # sourcery no-metrics
    ...
    zee = await edit_or_reply(event, "**╮ ❐ ... جــاࢪِ طــرد الشخــص عــام ... ❏╰**")
    start = datetime.now()
    user, reason = await get_user_from_event(event, zee)
    if not user:
        return
    if user.id == zeub.uid:
        return await edit_delete(zee, "**╮ ❐ ... عــذراً لا استطــيع طــرد نفســي ... ❏╰**")
    if user.id in ze_dev:
        return await edit_delete(zee, "**╮ ❐ ... عــذࢪاً .. لا استطــيع طــرد المطـورين ... ❏╰**")
    if user.id == 6581896306:
        return await edit_delete(zee, "**╮ ❐ ... عــذࢪاً .. لا استطــيع طــرد مطـور السـورس ... ❏╰**")
    san = await admin_groups(event.client)
    count = 0
    sandy = len(san)
    if sandy == 0:
    ...
    if reason:
        await zee.edit(
            f"[{user.first_name}](tg://user?id={user.id}) `was gkicked in {count} groups in {zetaken} seconds`!!\n**- الســبب :** `{reason}`"
        )
    else:
        await zee.edit(
            f"[{user.first_name}](tg://user?id={user.id}) `was gkicked in {count} groups in {zetaken} seconds`!!"
...
@zeub.ze_cmd(pattern=f"{ADMZ}(?:\s|$)([\s\S]*)")
...
@zeub.ze_cmd(pattern="رفع مالك(?:\s|$)([\s\S]*)")
async def promote(event):
    chat = await event.get_chat()
    admin = chat.admin_rights
    creator = chat.creator
...
@zeub.ze_cmd(pattern="اخفاء(?:\s|$)([\s\S]*)")
async def promote(event):
    chat = await event.get_chat()
    admin = chat.admin_rights
    creator = chat.creator
...
@zeub.ze_cmd(pattern=f"{UNADMZ}(?:\s|$)([\s\S]*)")
...
@zeub.ze_cmd(pattern=f"{BANN}(?:\s|$)([\s\S]*)")
async def _ban_person(event):
    ...
    zeevent = await edit_or_reply(event, "**╮ ❐... جـاࢪِ الحـظـࢪ ...❏╰**")
    try:
        await event.client(EditBannedRequest(event.chat_id, user.id, BANNED_RIGHTS))
    except BadRequestError:
        return await zeevent.edit(NO_PERM)
    reply = await event.get_reply_message()
    if reason:
        await zeevent.edit(
            f"**☣╎المستخـدم :** {_format.mentionuser(user.first_name ,user.id)}  \n**☣╎تم حظـࢪه بنجـاح ☑️**\n\n**☣╎السـبب :** `{reason}`"
    ...
    if BOTLOG:
        ...
        try:
            ...
            return await zeevent.edit(
                "`I dont have message nuking rights! But still he is banned!`"
            )


@zeub.ze_cmd(pattern=f"{UNBANN}(?:\s|$)([\s\S]*)")
async def nothanos(event):
    user, _ = await get_user_from_event(event)
    if not user:
        return
    zeevent = await edit_or_reply(event, "**╮ ❐.. جـاري الغاء حـظࢪه ..❏╰**")
    try:
        ...
        await zeevent.edit("`Uh oh my unban logic broke!`")
    except Exception as e:
        await zeevent.edit(f"**- خطــأ :**\n`{e}`")
...
@zeub.ze_cmd(pattern=f"{KICK}(?:\s|$)([\s\S]*)")
async def kick(event):
    ...
    zeevent = await edit_or_reply(event, "**╮ ❐... جـاࢪِ الطــࢪد ...❏╰**")
    try:
        await event.client.kick_participant(event.chat_id, user.id)
    except Exception as e:
        return await zeevent.edit(f"{NO_PERM}\n{e}")
    if reason:
        await zeevent.edit(
            f"**☣╎تم طــࢪد**. [{user.first_name}](tg://user?id={user.id})  **بنجــاح ✓**\n\n**☣╎السـبب :** {reason}"
        )
    else:
        await zeevent.edit(f"**☣╎تم طــࢪد**. [{user.first_name}](tg://user?id={user.id})  **بنجــاح ✓**")
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID,
            "#الـطــࢪد\n"
...
async def _iundlt(event):  # sourcery no-metrics
    ...
    zeevent = await edit_or_reply(event, "**- جـاري البحث عـن آخـر الاحداث انتظــر ...🔍**")
    ...
    if not flag:
        ...
        await edit_or_reply(zeevent, deleted_msg)
    else:
        main_msg = await edit_or_reply(zeevent, deleted_msg)
        for msg in adminlog:
            ruser = await event.client.get_entity(msg.old.from_id)
            _media_type = await media_type(msg.old)
            if _media_type is None:
