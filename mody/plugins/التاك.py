...
async def tagall(event):
  ...
  if mode == "by_cmd":
    ...
    usrnum = 0
    usrtxt = ""
    async for usr in zeub.iter_participants(event.chat_id):
      usrnum += 1
      ...
      if event.chat_id not in moment_worker:
        ...
        return
      if usrnum == 5:
        await zeub.send_message(event.chat_id, f"{usrtxt}\n\n- {msg}")
  ...
  if mode == "by_reply":
    ...
    async for usr in zeub.iter_participants(event.chat_id):
      ...
      if event.chat_id not in moment_worker:
        ...
        return
      if usrnum == 5:
        await zeub.send_message(event.chat_id, usrtxt, reply_to=msg)
        await asyncio.sleep(2)
        usrnum = 0
        usrtxt = ""
...
@zeub.ze_cmd(pattern="ايقاف التاك?(.*)")
