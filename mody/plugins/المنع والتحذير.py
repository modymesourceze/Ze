...
from . import zeub, BOTLOG_CHATID
...
@zeub.ze_cmd(incoming=True, groups_only=True)
...
@zeub.ze_cmd(pattern="تحذير(?:\s|$)([\s\S]*)")
async def _(event):
...
@zeub.ze_cmd(pattern="حذف التحذيرات(?: |$)(.*)")
async def _(event):
    reply_message = await event.get_reply_message()
    sql.reset_warns(reply_message.sender_id, event.chat_id)
