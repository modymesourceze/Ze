...
from ..helpers.utils import _zeutils, _format
...
async def spam_function(event, sandy, ze, sleeptimem, sleeptimet, DelaySpam=False):
    # sourcery no-metrics
    counter = int(ze[0])
    if len(ze) == 2:
        spam_message = str(ze[1])
        ...
        for _ in range(counter):
            ...
            await _zeutils.unsavegif(event, sandy)
...
async def spammer(event):
    ...
    ze = ("".join(event.text.split(maxsplit=1)[1:])).split(" ", 1)
    try:
        counter = int(ze[0])
    except Exception:
    ...
    await spam_function(event, sandy, ze, sleeptimem, sleeptimet)
...
@zeub.ze_cmd(pattern=f"{SPAM} ([\s\S]*)")
async def spammer(event):
    reply = await event.get_reply_message()
    ...
    await event.delete()
    addgvar("spamwork", True)
    await spam_function(event, reply, ze, sleeptimem, sleeptimet, DelaySpam=True)


@zeub.ze_cmd(pattern="تعبير مكرر$")
async def react_spam(event):
    msg = await event.get_reply_message()
    if not msg:
        return await edit_delete(event, "**- بالـرد على الرسـالة اولاً ...**", 10)
    zeevent = await edit_or_reply(event, "**- جـارِ بدء التفاعـلات انتظـر ...**")
...
@zeub.ze_cmd(pattern="ايقاف التكرار ?(.*)")
...
@zeub.ze_cmd(pattern="التكرار")
async def cmd(modyyy):
    await edit_or_reply(modyyy, ModySP_cmd)@