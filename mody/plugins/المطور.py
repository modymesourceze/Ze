...
ZelzalDV_cmd = (
    "[ᯓ 🔱 𝐒𝐎𝐔𝐑𝐂𝐄 𝐙𝐄 🔱 - اوامــر المطـور المســاعد](t.me/source_ze) .\n\n"
...
@zeub.ze_cmd(pattern="حذف_المطورين")
async def _(event):
...
async def _(event):  # sourcery no-metrics
    ...
    if input_str[0] == "آمن":
        zeevent = await edit_or_reply(event, "**☣╎تـم تفعيـل التحكـم للمطـوريـن لـ الاوامـر الآمـنـه .. بنجـاح🧑🏻‍💻✅**")
        ...
        for plugin in input_str:
            if plugin not in PLG_INFO:
                errors += (
                    f"`{plugin}` __There is no such plugin in your ZE__.\n"
                )
            else:
    ...
    if errors != "":
        output += "\n**- خطــأ :**\n" + errors
    msg = await edit_or_reply(zeevent, output)
    await event.client.reload(msg)
...
zeub.loop.create_task(_init())



# Copyright (C) 2023 Ze . All Rights Reserved
@zeub.ze_cmd(pattern="المساعد")
