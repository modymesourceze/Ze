...
async def lg1(userevent):
    ...
    if gvarstatus("ZE_FONTS") is None: # code by t.me/D_S_I
        ze_font = await get_font_file(userevent.client, "@T_Taiz")
    else:
        ze_font = gvarstatus("ZE_FONTS")
    ...
    if arabic_text: # code by t.me/D_S_I
    ...
    font = ImageFont.truetype(ze_font, font_size_)
...
async def lg1(userevent):
    ...
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
    if gvarstatus("ZE_FONTS") is None: # code by t.me/D_S_I
        ze_font = await get_font_file(userevent.client, "@S_SSQ")
    else: # code by t.me/D_S_I
        ze_font = gvarstatus("ZE_FONTS")
    if userevent.reply_to_msg_id:
        rply = await userevent.get_reply_message()
    ...
    if not arabic_text: # code by t.me/D_S_I
