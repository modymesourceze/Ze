...
from . import BOTLOG, BOTLOG_CHATID, Convert, zeub
...
async def gsearch(q_event):
    ...
    zeevent = await edit_or_reply(q_event, "**- جـارِ البحـث في جوجــل...**")
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                return await edit_delete(zeevent, f"**- خطـأ :**\n`{e}`", time=10)
    msg = ""
    for i in range(lim):
...
async def grs(event):
    ...
    if event.reply_to_msg_id:
        zeevent = await edit_or_reply(event, "Pre Processing Media")
        ...
        if previous_message.media:
            ...
            the_location = google_rs_response.headers.get("Location")
        await zeevent.edit("Found Google Result. Pouring some soup on it!")
        headers = {
        ...
        try:
            ...
            return await edit_delete(
                zeevent, "`Sorry. I am unable to find similar images`"
            )
        end = datetime.now()
...
<i>fetched in {ms} seconds</i>""".format(
        ...
        zeevent = event
    await edit_or_reply(zeevent, OUTPUT_STR, parse_mode="HTML", link_preview=False)
