...
from ..helpers.utils import _format, reply_id, _zeutils
from . import BOTLOG, BOTLOG_CHATID, zeub
...
async def _(event):
    ...
    zeevent = await edit_or_reply(event, "**╮ جـارِ البحث ؏ـن المقطـٓع الصٓوتـي... 🎧♥️╰**")
    ...
    try:
        ...
        pass
    try:
        stderr = (await _zeutils.runcmd(song_cmd))[1]
        # if stderr:
        # await zeevent.edit(f"**Error1 :** `{stderr}`")
        catname, stderr = (await _zeutils.runcmd(name_cmd))[:2]
        if stderr:
            return await zeevent.edit(f"**خطــأ :** `{stderr}`")
    ...
    await zeevent.edit("**╮ ❐ جـارِ التحميـل انتظـر قليلاً  ▬▭... 𓅫╰**")
    ...
    try:
        ...
        for files in (catthumb, song_file):
            if files and os.path.exists(files):
                os.remove(files)
    except ChatSendMediaForbiddenError as err: # Code By T.me/D_S_I
        await zeevent.edit("**- عـذراً .. الوسـائـط مغلقـه هنـا ؟!**")
        LOGS.error(str(err))
...
async def _(event):
    ...
    cat = base64.b64decode("QUFBQUFGRV9vWjVYVE5fUnVaaEtOdw==")
    zeevent = await edit_or_reply(event, "**╮ جـارِ البحث ؏ـن الفيديـو... 🎧♥️╰**")
    video_link = await yt_search(str(query))
    if not url(video_link):
        return await zeevent.edit(
            f"**☣╎عـذراً .. لـم استطـع ايجـاد** {query}"
    ...
    try:
        stderr = (await _zeutils.runcmd(video_cmd))[1]
        # if stderr:
        # return await zeevent.edit(f"**خطــأ :** `{stderr}`")
        catname, stderr = (await _zeutils.runcmd(name_cmd))[:2]
        if stderr:
    ...
    await zeevent.edit("**╮ ❐ جـارِ تحميـل الفيديـو انتظـر قليلاً  ▬▭... 𓅫╰**")
    catthumb = Path(f"{catname}.jpg")
    if not os.path.exists(catthumb):
        catthumb = Path(f"{catname}.webp")
    elif not os.path.exists(catthumb):
...
async def shazamcmd(event):
    ...
    zeevent = await edit_or_reply(event, "**- جـار تحميـل المقـطع الصـوتي ...**")
    name = "ze.mp3"
    try:
    ...
    if flag == "s":
        ...
        async with event.client.conversation(chat) as conv:
            try:
                ...
                await zeub(unblock("DeezerMusicBot"))
                purgeflag = await conv.send_message("/start")
...
# Code by T.me/D_S_I
@zeub.ze_cmd(pattern=".ff(?:\s|$)([\s\S]*)")
async def mody_song(event):
...
async def download_audio(event):
    ...
    zeevent = await edit_or_reply(event, "**⌔╎جـارِ التحميل انتظر قليلا ▬▭ ...**")
    reply_to_id = await reply_id(event)
    for url in urls:
        try:
        ...
        for _path in glob.glob(os.path.join(Config.TEMP_DIR, str(startTime), "*")):
            if _path.lower().endswith((".jpg", ".png", ".webp")):
                ...
                _fpath = _path
        if not _fpath:
            return await edit_delete(zeevent, "__Unable to upload file__")
        await zeevent.edit(
...
async def download_video(event):
    ...
    reply_to_id = await reply_id(event)
    for url in urls:
        ytdl_data = await ytdl_down(zeevent, video_opts, url)
        if ytdl_down is None:
            return
        try:
            f = pathlib.Path("ze_ytv.mp4")
            print(f)
            zethumb = pathlib.Path("ze_ytv.jpg")
            if not os.path.exists(zethumb):
                zethumb = pathlib.Path("ze_ytv.webp")
            if not os.path.exists(zethumb):
                zethumb = None
            await zeevent.edit(
                f"**╮ ❐ جـارِ التحضيـر للـرفع انتظـر ...𓅫╰**:\
            ...
            uploaded = await event.client.fast_upload_file(
                ...
                progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                    progress(
                        d, t, zeevent, c_time, "Upload :", file_name=ytdl_data["title"]
                    )
                ),
            )
            ul.close()
