...
async def post_to_telegraph(
    page_title,
    html_format_content,
    auth_name="🔱 𝐒𝐎𝐔𝐑𝐂𝐄 𝐙𝐄 🔱 - 🔱 سورس زد إي 🔱",
    auth_url="https://t.me/source_ze",
):
...
async def animator(media, mainevent, textevent=None):
    ...
    OldZe = await mainevent.client.download_media(media, Config.TEMP_DIR)
    file = await fileinfo(OldZe)
    h = file["height"]
    w = file["width"]
    ...
    await runcmd(
        f"ffmpeg -to 00:00:02.900 -i '{OldZe}' -vf scale={w}:{h} -c:v libvpx-vp9 -crf 30 -b:v 560k -maxrate 560k -bufsize 256k -an animate.webm"
    )  # pain
    os.remove(OldZed)
    return "animate.webm"
...
def higlighted_text(
    ...
    if font_name is None:
        font_name = "mody/helpers/styles/impact.ttf"
    font = ImageFont.truetype(font_name, font_size)
