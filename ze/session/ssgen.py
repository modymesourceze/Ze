...
def telethon_session():
    ...
    # logging in
    try:
        with TelegramClient(StringSession(), API_ID, API_HASH) as ze:
            print("Generating a string session for •Ze•")
            try:
                ze.send_message(
                    "me",
                    f"**- هـذا هو كـود سيشـن تيليثـون الخـاص بـك ☑️**:\n\n`{ze.session.save()}`\n\n**- لا تشـارك هـذا الكـود مع اي احـد ⚠️؟!**\n**- فقـط قم بنسخـه ولصقـه في مربعـات التنصيب 🛃**\n\n**- سـورس زد إي : @source_ze**",
                )
                print(
        ...
        print(er)
        print("If you think It as a Bug, Report to @D_S_I\n\n")
...
def pyro_session():
    ...
    with Client(
        name="ze", api_id=API_ID, api_hash=API_HASH, in_memory=True
    ) as pyro:
